...
def selectFiles():
    ...
    Name=input("please enter a descriptive name for the comparison. This will be the name of the directory containing the results of the comparison ")
    comparisonName=''
    comparisonName+=Name
    print(comparisonName)
    check=checkComparisonName(comparisonName)
    if check == comparisonName:
        pass
    else:
        comparisonName=check
    f=open("./comparisons/"+comparisonName+"/"+comparisonName, "w+")
    for line in files:
        f.write(line+"\n")
    f.close()
    os.chmod("./comparisons/"+comparisonName+"/"+comparisonName, 0o777)
    subprocess.run(["notepad", "./comparisons/"+comparisonName+"/"+comparisonName], universal_newlines=True)
    f=open("./comparisons/"+comparisonName+"/"+comparisonName, "r+")
...
#   sift through the data and put in the order of the number of drives used in that test, assuming that the tests start with 1 drive used
    ...
    for data in rawData:
        if data[0] not in tests:
            tests.append(data[0]) 
    graphStuff(compareTheseTests, tests, comparisonName)
    f=open("./comparisons"+"/"+comparisonName+"/"+comparisonName, "w+")
    for test in tests:
        IOPS=[]
        BW=[]
        for data in rawData:
            if data[0] == test:
                IOPS.append((data[1], data[3]))
                BW.append((data[2], data[3]))
...
def graphStuff(compareTheseTests, tests, comparisonName):
    BW_y_axes=[tests]
    IOPS_y_axes=[tests]
    ...
    plt.legend()
    plt.savefig("./comparisons/"+comparisonName+"/"+"graphs.png")
    
##################################################################################################
def checkComparisonName(comparisonName):
    
    while os.path.exists("./comparisons/"+comparisonName) != False:
        comparisonName = input("the chosen directory name is already in use, please enter a different name ")
    os.mkdir("./comparisons/"+comparisonName)    
